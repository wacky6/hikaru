...
class HighAvailabilityDanmakuStream extends EventEmitter {
    ...
    connectServers(servers) {
        ...
        connectServers.forEach(server => {
            ...
            dmk
                .on('open', () => this.emit('hdas', { event: 'open', server }))
                .on('close', () => this.emit('hdas', { event: 'close', server }))
                .on('suicide', () => this.emit('hdas', { event: 'suicide', server }))
                .on('heartbeat', () => this.emit('heartbeat', { event: 'heartbeat', server }))
        })
    }
